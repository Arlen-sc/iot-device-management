    ...
    showExecResult(result) {
        ...
        if (logs.length === 0) {
            ...
            logs.forEach(log => {
                ...
                // Parse object format: {timestamp, level, actionType, nodeName, message, data, durationMs}
                if (typeof log === 'string') {
                    // Fallback
                    html += '<div style="color:#a0e8af;margin-bottom:8px;word-break:break-all;border-bottom:1px dashed #333;padding-bottom:6px;">' + App.escapeHtml(log) + '</div>';
                    return;
                }
                ...
                var badges = '';
                if (log.actionType && log.actionType !== '-') badges += '<span style="background:rgba(255,255,255,0.1);padding:2px 6px;border-radius:4px;margin-right:8px;font-size:11px;color:#fff;">' + App.escapeHtml(log.actionType) + '</span>';
                if (log.nodeName && log.nodeName !== '-') badges += '<span style="background:rgba(255,255,255,0.1);padding:2px 6px;border-radius:4px;margin-right:8px;font-size:11px;color:#ddd;">' + App.escapeHtml(log.nodeName) + '</span>';
                if (log.durationMs != null) badges += '<span style="background:rgba(82,196,26,0.2);padding:2px 6px;border-radius:4px;margin-right:8px;font-size:11px;color:#a0d911;">耗时:' + log.durationMs + 'ms</span>';
